...
import {getColorFromArea} from "@/lib/utils.ts";
        ...
        <>
            <Tooltip delayDuration={300}>
                <TooltipTrigger className="w-full">
                    <div
                        className={`border-2 rounded-lg h-[52px] flex flex-col justify-center leading-none hover:brightness-105 relative ${hidden && 'opacity-50'} ${getStatusStyles(status)}`}>
                        <span style={{backgroundColor: getColorFromArea(name)}}
                              className={"h-2 w-4 rounded-full absolute top-1 right-1 border border-white"}></span>
                        {areaIsHuntable() ? (
                            name
                        ) : (
                            <>
